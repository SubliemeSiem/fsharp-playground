...
module Page =
    type Page () =
        static let scriptsBlock (scripts : string list) (path : string) = 
            List.fold (fun prev curr -> prev + "    <script src=\"" + path + curr + "\" async></script>\r\n") "" scripts

        static let styleBlock (styles : string list) (path : string) = 
        ...
        static let pageLinks (links : string list) (path : string) = 
            List.fold (fun prev curr -> prev + "            <div class=\"pagelink\" onclick=\"openPage('" + path + curr + "'.toLowerCase(), '" + curr + "', true);\" id=\"" + curr + "\">" + curr + "</div>\r\n") "" links

        static let messageBlock (messages : string list) =
            List.fold (fun prev curr -> prev + "            <div class=\"message\">\r\n                " + curr + "\r\n            </div>\r\n") "" messages

        static member ContentResponse (title : string) (page : string) (html : string) =
            "{\"title\": \"" + title + "\", \"page\": \"" + page + "\", \"html\": \"" + html + "\"}"

        static member Html (title : string) (path : string) (scripts : string list) (styles : string list) (inlineStyle : string) (links : string list) (content : string) (messages : string list) = 
